                                    ...
                                    <div class="flex items-center mb-4 space-x-4">
                                        <div class="flex-1">
                                            ...
                                            <select id="address_select_{{ $index }}" class="address-select mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                ...
                                                @foreach($order->user->addresses as $userAddress)
                                                    <option value="{{ $userAddress->id }}" {{ $userAddress->id == $address->id ? 'selected' : '' }}>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        addressSelects.forEach(select => {
            select.addEventListener('change', function () {
                const selectedAddress = users.find(user => user.id == {{ $order->user_id }}).addresses.find(address => address.id == this.value);
                if (selectedAddress) {
                    // Populate the address fields based on the selected address
                    const parentDiv = this.closest('.shipping-address');
                    parentDiv.querySelector(`#address_${parentDiv.dataset.index}`).value = selectedAddress.address;
                    parentDiv.querySelector(`#city_${parentDiv.dataset.index}`).value = selectedAddress.city;
                    parentDiv.querySelector(`#district_${parentDiv.dataset.index}`).value = selectedAddress.district;
                    parentDiv.querySelector(`#ward_${parentDiv.dataset.index}`).value = selectedAddress.ward;
                    parentDiv.querySelector(`#phone_${parentDiv.dataset.index}`).value = selectedAddress.phone_number;
                }
            });
        });
        ...
        document.addEventListener('change', function (event) {
            if (event.target.classList.contains('address-select')) {
                ...
                const selectedUser = {id: '{{ $order->user_id }}', addresses: @json($order->user->addresses)};
        ...
        document.getElementById('add_shipping_address').addEventListener('click', function () {
            ...
            updateAddressOptions(@json($order->user->addresses));

            shippingAddressCount++;
        });

        document.querySelectorAll('.shipping-address').forEach(addressDiv => {
            addressDiv.querySelectorAll('.product-row').forEach(row => {
        ...
        function addProduct(index) {
            ...
            if (selectedOption.value) {
                ...
                if (existingRow) {
                    ...
                    row.classList.add('hover:bg-gray-100', 'product-row');
