                    ...
                    <form method="POST" action="{{ route('users.update', $user->id) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="is_active" id="is_active"
                                class="mt-2 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                required>
                                <option value="1" {{ old('is_active', $user->is_active) ? 'selected' : '' }}>Kích hoạt
                                </option>
                                <option value="0" {{ !old('is_active', $user->is_active) ? 'selected' : '' }}>Vô hiệu
                                    hóa</option>
                            </select>
                            @error('is_active')
                                <p class="text-red-600 mt-1">{{ $message }}</p>
                        ...
                        <div class="flex justify-end space-x-4">
                            <a href="{{ route('users.index') }}"
                                class="py-2 px-4 bg-red-600 text-white font-semibold rounded-md shadow-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                                Hủy
                            </a>
                            <button type="submit"
